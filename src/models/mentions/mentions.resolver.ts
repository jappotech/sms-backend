...
export class MentionsResolver {
  ...
  @AllowAuthenticated()
  @Mutation(() => Mention)
  async updateMention(@Args('updateMentionInput') args: UpdateMentionInput, @GetUser() user: GetUserType) {
    const mention = await this.prisma.mention.findUnique({ where: { id: args.id } })
  ...
  @AllowAuthenticated()
  @Mutation(() => Mention)
  async removeMention(@Args() args: FindUniqueMentionArgs, @GetUser() user: GetUserType) {
